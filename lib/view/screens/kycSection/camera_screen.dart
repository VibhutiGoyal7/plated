...
class _DocImageScreenState extends State<DocImageScreen> {
  ...
  void initState() {
    ...
    if (nonCapitalizeString(docType) == nonCapitalizeString("passport")) {
      isBothSides = false;
    } else if (nonCapitalizeString(docType) ==
        nonCapitalizeString("national_id")) {
      isBothSides = true;
    } else if (nonCapitalizeString(docType) ==
        nonCapitalizeString("driving_licence")) {
      isBothSides = true;
    } else if (nonCapitalizeString(docType) ==
        nonCapitalizeString("video_kyc_clip")) {
      isBothSides = false;
    } else if (nonCapitalizeString(docType) ==
        nonCapitalizeString("address_kyc")) {
      isBothSides = false;
    } else if (nonCapitalizeString(docType) ==
        nonCapitalizeString("bank_statement")) {
      isBothSides = false;
    }
    imageName = "kyc_file";
  }
 ...
 /* Future<Widget> submitKycDocResponse(
        ...
        child: Scaffold(
          ...
          body: Stack(
            children: [
                            ...
                            child: Column(
                              children: [
                                _buildImageContainer(
                                  label: "Front Side",
                                  imageClicked: frontImageClicked,
                                  onPressed: () => onPressedFrontImage(),
                                  image: frontImg,
                                ),
                                if (isBothSides)
                                  _buildImageContainer(
                                    label: "Back Side",
                                    imageClicked: backImgClicked,
                                    onPressed: () => onPressedBackImage(),
                                    image: backImg,
                                  ),
                              ],
                            ),
              ...
              if (isLoading)
                Center(
                  child: CircularProgressIndicator(color: isDarkMode ? AppColor.WHITE : AppColor.PRIMARY,),
                ),
            ],
          ),
        )
  ...
  Future<File> processImagesAndReturnFile(File? files, File? backImg) async {
    ...
    final file =
        await File('${directory.path}/compressed_screenshot.jpg').create();
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  if (isInputValid) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Capture document image'),
                        duration: maxDuration,
                      ),
                    );
                  }
                ...
                style: ElevatedButton.styleFrom(
                    ...
                    backgroundColor:
                        isInputValid ? Theme.of(context).cardColor : Colors.white,
                    elevation: 3,
  ...
  // Function to build a document side container
  Widget _buildImageContainer({
    required String label,
    required bool imageClicked,
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        alignment: Alignment.center,
        height: screenHeight * (isBothSides ? 0.35 : 0.4),
        width: double.infinity,
        decoration: BoxDecoration(border: Border.all(width: 0.5,color: isDarkMode ? Colors.grey : Colors.black54)),
        child: imageClicked
            ? ClipRRect(
          child: Image.file(image!, width: screenWidth, fit: BoxFit.fill),
        )
            : _buildPlaceholder(label,isBothSides),
      ),
    );
  }

  // Function to build placeholder for image container
  Widget _buildPlaceholder(String label,bool isBothSides) {
    return Container(
      ...
      alignment: Alignment.center,
      child: label=="Front Side"?
      SvgPicture.asset("assets/documet_front_side.svg",
          height: screenHeight*0.3,
          width: screenWidth*0.7,
          ...
          :
      SvgPicture.asset("assets/document_back_side.svg",
