...
class BdOneApiServicesImpl implements BdOneApiServices {
  BdOneApiService _BDPayService = BdOneApiService();
  ...
  Future<T> _fetchData<T>(
    ...
    try {
      ...
      if (method == GET) {
        response = await _BDPayService.getResponse(url);
      } else if (method == POST) {
        response = await _BDPayService.postResponse(url, requestBody);
      } else if (method == PUT && !isSetupAccount) {
        response = await _BDPayService.putResponse(
            url, requestBody); // Assuming you have a putResponse method
      } else if (method == PUT && isSetupAccount) {
        print("method Token: $isSetupAccount $token");
        print("Request: $requestBody");
        response = await _BDPayService.putSetUpAccountResponse(
            url, requestBody, token); // Assuming you have a putResponse method
      } else if (method == DELETE) {
        print("DELETE: $DELETE");
        response = await _BDPayService.deleteResponse(url);
      } else {
